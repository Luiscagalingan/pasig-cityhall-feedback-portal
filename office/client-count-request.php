...
render_dashboard_start('Request Client Count from Administrator', 'client_request');
page_header('Request Client Count from Administrator', 'Send a request to an Administrator to verify your assisted-client total for a selected calendar year.');
?>
<section class="card"><p class="muted">The Administrator will verify eligible assisted-survey records owned by your account in your assigned office, from January 1 through December 31.</p><form method="post"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><label>Calendar Year<input type="number" name="requested_year" min="1900" max="9998" value="<?= e(date('Y')) ?>" required></label><button class="btn">Send Request to Administrator</button></form></section>
