...
$validRoles=['office_head','office_staff'];
$where=['u.office_id=?'];
$params=[$officeId];
...
page_header($user['office_code'].' System Audit','Makikita rito ang lahat ng naka-log na gawain ng Head at Staff (kasama ang historical roles) sa iyong opisina.');
?>
<div class="tabs"><a class="tab <?= $roleFilter===''?'active':'' ?>" href="?<?= e(http_build_query(array_filter(['q'=>$q,'action_filter'=>$actionFilter]))) ?>">Lahat</a><?php foreach(['office_head'=>'Office Head','office_staff'=>'Office Staff'] as $roleKey=>$roleLabel):?><a class="tab <?= $roleFilter===$roleKey?'active':'' ?>" href="?<?= e(http_build_query(array_filter(['q'=>$q,'action_filter'=>$actionFilter,'role_filter'=>$roleKey]))) ?>"><?= e($roleLabel) ?></a><?php endforeach;?></div>
...
<form class="filters wide" method="get"><input name="q" value="<?= e($q) ?>" placeholder="Hanapin ang user, action, detalye, o IP"><select name="role_filter"><option value="">Lahat ng role</option><?php foreach(['office_head'=>'Office Head','office_staff'=>'Office Staff'] as $roleKey=>$roleLabel):?><option value="<?= $roleKey ?>" <?= $roleFilter===$roleKey?'selected':'' ?>><?= e($roleLabel) ?></option><?php endforeach;?></select><select name="action_filter"><option value="">Lahat ng action</option><?php foreach($actions as $action):?><option value="<?= e($action) ?>" <?= $actionFilter===$action?'selected':'' ?>><?= e(status_label($action)) ?></option><?php endforeach;?></select><button class="btn">I-filter</button></form>
<div class="table-wrap"><table><thead><tr><th>Petsa at Oras</th><th>User</th><th>Role</th><th>Action</th><th>Detalye</th><th>IP Address</th></tr></thead><tbody><?php foreach($logs as $log):?><tr><td><?= e(date('M d, Y h:i A',strtotime($log['created_at']))) ?></td><td><strong><?= e($log['full_name']) ?></strong><br><span class="muted"><?= e($log['username']) ?></span></td><td><?= badge((!active_application_role((string)$log['role']) ? 'Legacy role (retired)' : status_label($log['role'])),$log['role']==='office_head'?'neutral':'positive') ?></td><td><?= e(status_label($log['action'])) ?></td><td><?= e($log['details']) ?></td><td><?= e($log['ip_address']) ?></td></tr><?php endforeach;?><?php if(!$logs):?><tr><td colspan="6" class="empty-state">Walang audit record na tumutugma sa filter.</td></tr><?php endif;?></tbody></table></div><?= pagination_links($p) ?></section>
