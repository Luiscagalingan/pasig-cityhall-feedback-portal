...
// Opening Sentiment Review acknowledges its related notification for this user.
try {
    db()->prepare("UPDATE notifications SET read_at=NOW() WHERE user_id=? AND read_at IS NULL AND (link_url='review.php' OR link_url LIKE '%review.php%')")
        ->execute([(int)$user['id']]);
} catch (Throwable) {}

if (($_GET['export'] ?? '') === 'training_csv') {
    if (!$isAdmin) { http_response_code(403); exit('Training export is Administrator-only.'); }
...
page_header('Sentiment Review',$isAdmin ? 'Verify low-confidence or fallback classifications, preserve corrections, and approve examples for retraining.' : 'View and monitor feedback reviews.',$isAdmin ? '<a class="btn secondary" href="?export=training_csv">Export Reviewed Training CSV</a>' : '');
?>
<div class="sentiment-review-page">
...
<section class="card"><div class="table-wrap"><table><thead><tr><th>Assisted By</th><th>Timestamp</th><th>Client Number</th><th>Surname</th><th>Given Name</th><th>Middle Name</th><th>Sex</th><th>Age</th><th>Types of Client</th><th>Transaction / Service</th><th>Bilis ng Serbisyo (Timeliness)</th><th>Pakikitungo sa Kliyente (Client Handling)</th><th>Kalidad ng Serbisyo (Quality of Service)</th><th>Overall Satisfaction</th><th>Prediction</th><th>Weighted Result</th><th>Human Review / Correction</th></tr></thead><tbody><?php foreach($rows as $r): ?><tr class="<?= $r['is_void']?'void-row':'' ?>"><td><?= e($r['assisted_by'] ?: 'Unassigned') ?><br><small class="muted">#<?= (int)$r['id'] ?> · <?= e($r['office_code']) ?></small></td><td><?= e($r['visit_date']) ?></td><td><?= e($r['client_number'] ?: '—') ?></td><td><?= e($r['surname'] ?: '—') ?></td><td><?= e($r['given_name'] ?: '—') ?></td><td><?= e($r['middle_initial'] ?: '—') ?></td><td><?= e($r['sex']) ?></td><td><?= (int)$r['age'] ?></td><td><?= e($r['client_type']) ?></td><td><?= e($r['service_received']) ?><p class="help"><?= e($r['comment']) ?></p><?php if($r['is_void']): ?><?= badge('Void','negative') ?><br><small><?= e($r['void_reason']) ?></small><?php endif; ?></td><td><?= (int)$r['timeliness_rating'] ?></td><td><?= (int)$r['client_handling_rating'] ?></td><td><?= (int)$r['quality_rating'] ?></td><td><?= (int)$r['overall_rating'] ?></td><td><?= badge(status_label($r['sentiment']),$r['sentiment']) ?><br><span class="muted"><?= number_format((float)$r['sentiment_confidence']*100,2) ?>% · <?= e($r['sentiment_source']) ?></span><br><?= badge(status_label($r['review_status']),$r['review_status']==='needs_review'?'negative':'positive') ?><br><small><?= e($r['model_version']) ?></small></td><td><strong><?= number_format((float)$r['final_score'],2) ?>%</strong><br><span class="muted">Rating <?= number_format((float)$r['rating_percent'],2) ?>% × <?= trim((string)$r['comment']) !== '' ? '90%' : '100%' ?><?php if(trim((string)$r['comment']) !== ''): ?><br>Comment <?= number_format((float)$r['comment_score'],2) ?>% × 10%<?php else: ?><br>No comment — ratings only<?php endif; ?></span></td><td><?php if (!$isAdmin): ?>View only<?php elseif(!$r['is_void']): ?><form method="post" class="review-form"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="correct"><input type="hidden" name="feedback_id" value="<?= (int)$r['id'] ?>"><select name="sentiment" required><?php foreach(['positive','neutral','negative'] as $v): ?><option value="<?= $v ?>" <?= $r['sentiment']===$v?'selected':'' ?>><?= e(status_label($v)) ?></option><?php endforeach; ?></select><textarea name="reviewer_notes" required minlength="5" placeholder="Explain why this is the verified label."><?= e($r['reviewer_notes']) ?></textarea><label class="checkbox-row"><input type="checkbox" name="add_training" value="1"><span>Add the de-identified comment and verified label to the retraining candidates.</span></label><button class="btn small">Verify & Recalculate</button></form><details><summary class="btn danger small">Void Incorrect Record</summary><form method="post" class="details-form"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="void"><input type="hidden" name="feedback_id" value="<?= (int)$r['id'] ?>"><textarea name="void_reason" required minlength="5" placeholder="Reason for voiding"></textarea><button class="btn danger small">Confirm Void</button></form></details><?php else: ?><form method="post"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="unvoid"><input type="hidden" name="feedback_id" value="<?= (int)$r['id'] ?>"><button class="btn success small">Restore Record</button></form><?php endif; ?><?php if($r['reviewer_name']): ?><p class="help">Reviewed by <?= e($r['reviewer_name']) ?> on <?= e(date('M d, Y h:i A',strtotime($r['reviewed_at']))) ?></p><?php endif; ?></td></tr><?php endforeach; ?><?php if(!$rows): ?><tr><td colspan="17" class="empty-state">No feedback matches the review filters.</td></tr><?php endif; ?></tbody></table></div><?= pagination_links($p) ?></section>
</div>
<?php if(!$isAdmin): ?><script>document.querySelectorAll('.review-form, .details-form, .review-form + details, td:last-child > form').forEach((item)=>item.remove());</script><?php endif; ?>
