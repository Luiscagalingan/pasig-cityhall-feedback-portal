...
{
    return [
        ['overview','Overview','admin/dashboard.php','grid'],
        ['feedback','Feedback Records','admin/feedback.php','message'],
        ['actions','Action Management','admin/actions.php','check'],
        ['offices','Offices','admin/offices.php','building'],
        ['heads','Manage Heads','admin/heads.php','users'],
        ['staff','Manage Staff','admin/staff.php','users'],
        ['reports','Reports & Export','admin/reports.php','report'],
        ['account','My Account','account.php','users'],
...
{
    $nav = [
        ['overview','Overview','office/dashboard.php','grid'],
        ['feedback','Feedback Records','office/feedback.php','message'],
...
<link rel="stylesheet" href="<?= e(app_url('assets/css/app.css')) ?>?v=1">
</head>
...
<aside class="sidebar">
  <a class="brand" href="<?= e(app_url(dashboard_path($user))) ?>">
    <div class="seal">P</div>
    <div><strong>Pasig City Hall</strong><small>Service Feedback</small></div>
  </a>
  <div class="role-card">
    <span><?= e(initials($user['full_name'])) ?></span>
    <div><strong><?= e($user['full_name']) ?></strong><small><?= e(status_label($user['role'])) ?></small></div>
  </div>
  <nav>
  <?php foreach ($nav as [$key,$label,$path,$iconName]): ?>
    <a class="nav-link <?= $active === $key ? 'active' : '' ?>" href="<?= e(app_url($path)) ?>">
      <?= icon($iconName) ?><span><?= e($label) ?></span>
      <?php if ($key === 'actions' && $metrics['needs_action'] > 0): ?><b><?= (int)$metrics['needs_action'] ?></b><?php endif; ?>
    </a>
  <?php endforeach; ?>
  </nav>
  <a class="nav-link logout-link" href="<?= e(app_url('logout.php')) ?>" data-confirm-logout><?= icon('logout') ?><span>Logout</span></a>
</aside>
<div class="app-shell">
<header class="topbar">
  <button class="icon-btn mobile-only" data-sidebar-open aria-label="Open menu"><?= icon('menu') ?></button>
  <div><small><?= $user['role'] === 'admin' ? 'SYSTEM ADMINISTRATION' : 'OFFICE WORKSPACE' ?></small><strong><?= e($user['role'] === 'admin' ? 'Administrator Console' : $user['office_name']) ?></strong></div>
  <div class="topbar-actions">
    <span class="scope-pill"><?= e($user['role'] === 'admin' ? 'ALL OFFICES' : $user['office_code']) ?></span>
    <button class="theme-toggle" data-theme-toggle><?= icon('sun') ?><span>Light</span></button>
  </div>
</header>
