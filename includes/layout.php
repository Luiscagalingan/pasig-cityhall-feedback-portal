...
<div class="app-shell"><header class="topbar"><button class="icon-btn mobile-only" data-sidebar-open aria-label="Open menu"><?= icon('menu') ?></button><div><small><?= $user['role'] === 'admin' ? 'SYSTEM ADMINISTRATION' : 'OFFICE WORKSPACE' ?></small><strong><?= e($user['role'] === 'admin' ? 'Administrator Console' : $user['office_name']) ?></strong></div><div class="topbar-actions"><div class="notification-menu"><button type="button" class="icon-btn notification-link" data-notification-toggle aria-label="Open notifications" aria-expanded="false"><?= icon('bell') ?><?php if($unread): ?><b><?= $unread ?></b><?php endif; ?></button><section class="notification-preview" data-notification-preview aria-hidden="true"><div class="notification-preview-head"><div><strong>Notifications</strong><small><?= $unread ?> unread</small></div><a href="<?= e(app_url('notifications.php')) ?>">View all</a></div><div class="notification-preview-list"><?php foreach($notificationPreview as $item): ?><a class="notification-preview-item <?= $item['read_at']?'':'unread' ?>" href="<?= e(app_url('notifications.php')) ?>"><span><?= e($item['title']) ?></span><p><?= e(mb_strimwidth($item['message'],0,90,'…')) ?></p><small><?= e(date('M d, h:i A',strtotime($item['created_at']))) ?></small></a><?php endforeach; ?><?php if(!$notificationPreview): ?><div class="empty-state">No notifications yet.</div><?php endif; ?></div></section></div><span class="scope-pill"><?= e($scopeLabel) ?></span></div></header><main class="page-content">
