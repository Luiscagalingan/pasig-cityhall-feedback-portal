...
{
    $flash = consume_flash(); ?>
<!doctype html><html lang="en" data-theme="dark"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title><?= e($title) ?> | <?= e(APP_SHORT_NAME) ?></title><link rel="stylesheet" href="<?= e(app_url('assets/css/app.css')) ?>?v=3"></head><body class="<?= e($bodyClass) ?>">
<?php if ($flash): ?><div class="toast <?= e($flash['type']) ?>" data-toast><?= e($flash['message']) ?></div><?php endif;
}

function render_public_end(): void
{ ?>
<script src="<?= e(app_url('assets/js/app.js')) ?>?v=3"></script></body></html>
<?php }
...
function render_dashboard_start(string $title, string $active): array
{
    ...
    ?>
<!doctype html><html lang="en" data-theme="dark"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title><?= e($title) ?> | <?= e(APP_SHORT_NAME) ?></title><link rel="stylesheet" href="<?= e(app_url('assets/css/app.css')) ?>?v=3"></head><body class="dashboard-body">
<div class="sidebar-overlay" data-sidebar-close></div><aside class="sidebar"><a class="brand" href="<?= e(app_url(dashboard_path($user))) ?>"><div class="seal">P</div><div><strong>Pasig City Hall</strong><small>Service Feedback</small></div></a><div class="role-card"><span><?= e(initials($user['full_name'])) ?></span><div><strong><?= e($user['full_name']) ?></strong><small><?= e(status_label($user['role'])) ?></small></div></div><nav>
...
{ ?>
</main></div><script src="<?= e(app_url('assets/js/app.js')) ?>?v=3"></script></body></html>
<?php }
