...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['consent'])) $errors[] = 'Confirm that you agree to submit the feedback.';

    if (!$errors) {
        $prediction = predict_sentiment($comment);
        $scores = compute_feedback_scores($ratings, $prediction['label']);
        $stmt = db()->prepare("INSERT INTO feedback (office_id, visit_date, sex, age, client_type, service_received, timeliness_rating, client_handling_rating, quality_rating, overall_rating, comment, sentiment, sentiment_confidence, sentiment_source, average_rating, rating_percent, comment_score, final_score, source, submitted_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, 'public_survey', NOW())");
        $stmt->execute([(int)$office['id'],$visitDate,$sex,$age,$clientType,$service,(int)$ratings[0],(int)$ratings[1],(int)$ratings[2],(int)$ratings[3],$comment,$prediction['label'],$prediction['confidence'],$prediction['source'],$scores['average_rating'],$scores['rating_percent'],$scores['comment_score'],$scores['final_score']]);
        ...
        audit(null, 'public_feedback_submit', 'Office ' . $office['code'] . ', feedback #' . $feedbackId);
...
<div class="survey-shell">
  <div class="survey-banner"><span class="eyebrow"><?= e($office['code']) ?> FEEDBACK FORM</span><h1><?= e($office['name']) ?></h1><p class="muted">Rate the service you received. Your written comment is required and will be classified by the trained SVM sentiment model.</p></div>
  <form method="post" class="survey-form"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="office_code" value="<?= e($office['code']) ?>">
    ...
    <h2 class="section-title">Client and transaction details</h2>
    ...
    <h2 class="section-title">Service ratings</h2><p class="help">1 = Strongly Disagree, 2 = Disagree, 3 = Agree, 4 = Strongly Agree</p>
    <div class="rating-grid">
    <?php $items=['timeliness'=>'Timeliness — the time required to process the request or transaction.','client_handling'=>'Client Handling — courtesy and professionalism shown by personnel.','quality'=>'Quality of Service — accuracy, completeness, and reliability of the service.','overall'=>'Overall Satisfaction — your overall satisfaction with the service received.']; foreach($items as $name=>$label): ?>
      <div class="rating-item"><strong><?= e($label) ?> <span class="required">*</span></strong><div class="rating-options"><?php for($i=1;$i<=4;$i++): ?><label><input type="radio" name="<?= e($name) ?>" value="<?= $i ?>" <?= ((int)($_POST[$name] ?? 0)===$i)?'checked':'' ?> required><span><?= $i ?> · <?= ['','Strongly Disagree','Disagree','Agree','Strongly Agree'][$i] ?></span></label><?php endfor; ?></div></div>
    <?php endforeach; ?></div>
    <h2 class="section-title">Required written comment</h2><div class="form-group"><label>Comment or Suggestion <span class="required">*</span></label><textarea name="comment" minlength="5" maxlength="3000" required placeholder="Describe your experience in English, Filipino, or Taglish."><?= e($_POST['comment'] ?? '') ?></textarea><div class="help">The SVM model classifies the full comment as positive, neutral, or negative. Authorized personnel still review the original comment before taking action.</div></div>
    <label class="checkbox-row"><input type="checkbox" name="consent" value="1" required><span>I voluntarily submit this feedback and understand that it will be used for service-quality monitoring.</span></label>
